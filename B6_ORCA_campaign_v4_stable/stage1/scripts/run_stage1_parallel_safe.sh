...
if [[ -z "${WORKERS:-}" ]]; then
  WORKERS="$(python3 "$ROOT_DIR/scripts/safe_plan.py" | python3 -c 'import sys,json; print(json.load(sys.stdin)["recommended_workers"])')"
fi
echo "Using ORCA_BIN=$ORCA_BIN"; echo "Using WORKERS=$WORKERS"; echo "Stable mode: nprocs=1 per job"
...
wait || true
python3 "$ROOT_DIR/scripts/collect_stage1_results.py"
echo "Done. See $ROOT_DIR/results/summary.csv and $ROOT_DIR/results/best_stage1.json"
